"""
Feature Engineering - TheArena FreqAI Server (Enhanced)
========================================================
Merges the best of features_improved.py into the main pipeline:
  - EMA 13/50/200 alongside SMAs
  - Marubozu + spinning top candle patterns
  - Volume spike + ROC
  - Momentum (10-period)
  - Stochastic RSI
  - Williams %R
  - Improved price action (full lookback consistency)
  - All original features preserved (no breaking changes)
...
class FeatureEngineer:
    ...
    @staticmethod
    def calculate_stoch_rsi(close: np.ndarray, period: int = 14,
                             smooth_k: int = 3, smooth_d: int = 3) -> Tuple[np.ndarray, np.ndarray]:
        rsi = FeatureEngineer.calculate_rsi(close, period)
        ...
        return k.fillna(0.5).values, d.fillna(0.5).values

    @staticmethod
    def calculate_williams_r(high: np.ndarray, low: np.ndarray,
                              close: np.ndarray, period: int = 14) -> np.ndarray:
        h_series = pd.Series(high).rolling(period).max()
        l_series = pd.Series(low).rolling(period).min()
        wr = -100 * (h_series - close) / (h_series - l_series + 1e-10)
        return wr.fillna(-50).values
    ...
    def calculate_advanced_features(df: pd.DataFrame,
        ...
        # ── 2. EMA FEATURES (NEW — 8) ─────────────────────────────────────
        ema_13  = FeatureEngineer.calculate_ema(close, 13)
        ema_50  = FeatureEngineer.calculate_ema(close, 50)
        ema_200 = FeatureEngineer.calculate_ema(close, 200)

        features['ema13_above_ema50']    = 1.0 if ema_13[-1]  > ema_50[-1]  else 0.0
        features['ema50_above_ema200']   = 1.0 if ema_50[-1]  > ema_200[-1] else 0.0
        features['price_above_ema13']    = 1.0 if cp > ema_13[-1]  else 0.0
        features['price_above_ema200']   = 1.0 if cp > ema_200[-1] else 0.0
        features['ema13_ema50_distance'] = float(np.clip(_safe_div(ema_13[-1] - ema_50[-1], ema_50[-1] + 1e-10), -0.1, 0.1))
        features['ema_golden_cross']     = 1.0 if (len(ema_13) > 2 and ema_13[-1] > ema_50[-1] and ema_13[-2] <= ema_50[-2]) else 0.0
        features['ema_death_cross']      = 1.0 if (len(ema_13) > 2 and ema_13[-1] < ema_50[-1] and ema_13[-2] >= ema_50[-2]) else 0.0
        features['price_ema200_distance']= float(np.clip(_safe_div(cp - ema_200[-1], ema_200[-1] + 1e-10), -0.2, 0.2))
        ...
        # ── 5. STOCHASTIC RSI (NEW — 4) ───────────────────────────────────
        stoch_k, stoch_d = FeatureEngineer.calculate_stoch_rsi(close, 14, 3, 3)
        features['stoch_rsi_k']          = float(np.clip(stoch_k[-1], 0, 1))
        features['stoch_rsi_d']          = float(np.clip(stoch_d[-1], 0, 1))
        features['stoch_rsi_overbought'] = 1.0 if stoch_k[-1] > 0.8 else 0.0
        features['stoch_rsi_oversold']   = 1.0 if stoch_k[-1] < 0.2 else 0.0

        # ── 6. WILLIAMS %R (NEW — 3) ──────────────────────────────────────
        wr = FeatureEngineer.calculate_williams_r(high, low, close, 14)
        features['williams_r']            = float(np.clip((wr[-1] + 100) / 100, 0, 1))
        features['williams_r_overbought'] = 1.0 if wr[-1] > -20 else 0.0
        features['williams_r_oversold']   = 1.0 if wr[-1] < -80 else 0.0

        # ── 7. MOMENTUM INDICATOR (NEW — 2) ───────────────────────────────
        if len(close) > 10:
            mom_10 = float(close[-1] - close[-11])
            mom_10_pct = _safe_div(mom_10, close[-11] + 1e-10)
            features['momentum_10']     = float(np.clip(mom_10_pct, -0.1, 0.1))
            features['momentum_10_pos'] = 1.0 if mom_10 > 0 else 0.0
        else:
            features['momentum_10']     = 0.0
            features['momentum_10_pos'] = 0.0
        ...
        vol_roc    = _safe_div(volume[-1] - vol_sma_20, vol_sma_20 + 1e-10) * 100
        ...
        features['volume_spike']        = 1.0 if vol_roc > 50 else 0.0
        features['volume_roc']          = float(np.clip(vol_roc / 100, -1, 1))

        # ── 10. SUPPORT / RESISTANCE (6) ──────────────────────────────────
        ...
        # NEW candle patterns from features_improved
        features['candle_marubozu']       = 1.0 if (upper_wick < c_range * 0.05 and lower_wick < c_range * 0.05) else 0.0
        features['candle_spinning_top']   = 1.0 if (body < c_range * 0.3 and upper_wick > body and lower_wick > body) else 0.0
        features['candle_engulfing_bear'] = 1.0 if (len(close) >= 2 and close[-1] < open_[-1] and
                                                     close[-1] < open_[-2] and open_[-1] > close[-2]) else 0.0
        features['candle_body_ratio']     = float(np.clip(_safe_div(body, c_range), 0, 1))
        ...
        # NEW — full lookback consistency from features_improved
        lookback = min(5, len(high))
        recent_highs = high[-lookback:]
        recent_lows  = low[-lookback:]
        features['pa_higher_high'] = 1.0 if (len(recent_highs) > 1 and np.all(np.diff(recent_highs) > 0)) else 0.0
        features['pa_lower_low']   = 1.0 if (len(recent_lows) > 1  and np.all(np.diff(recent_lows) < 0))  else 0.0

        # ── 14. MEAN REVERSION / BOLLINGER (6) ───────────────────────────
        ...
        features['bb_width']              = float(np.clip(_safe_div(bb_range, bb_mid + 1e-10), 0, 0.2))
        features['bb_squeeze']            = 1.0 if features['bb_width'] < 0.02 else 0.0
        ...
        if ts is not None:
            try:
                ...
                features['is_overlap']        = 1.0 if 13 <= hour < 17 else 0.0
    ...
    def get_feature_columns() -> List[str]:
        return [
            ...
            # EMA (8) NEW
            'ema13_above_ema50','ema50_above_ema200',
            'price_above_ema13','price_above_ema200',
            'ema13_ema50_distance','ema_golden_cross','ema_death_cross',
            'price_ema200_distance',
            # ADX / Regime (9)
            ...
            # Stochastic RSI (4) NEW
            'stoch_rsi_k','stoch_rsi_d','stoch_rsi_overbought','stoch_rsi_oversold',
            # Williams %R (3) NEW
            'williams_r','williams_r_overbought','williams_r_oversold',
            # Momentum 10 (2) NEW
            'momentum_10','momentum_10_pos',
            # Volatility (8)
            ...
            'candle_marubozu','candle_spinning_top','candle_engulfing_bear',
            'candle_body_ratio',
            # Price Action (6)
...
def build_mtf_features(base_features: Dict[str, float],
    ...
    htf_keys = [
        ...
        # NEW HTF keys
        'ema13_above_ema50', 'ema50_above_ema200',
        'stoch_rsi_k', 'stoch_rsi_overbought', 'stoch_rsi_oversold',
    ...
    if higher_features:
        ...
        # NEW — EMA confluence
        merged['mtf_ema_bull'] = (
            1.0 if (base_features.get('ema13_above_ema50', 0) and
                    higher_features.get('ema13_above_ema50', 0))
            else 0.0
        )
        merged['mtf_ema_bear'] = (
            1.0 if (not base_features.get('ema13_above_ema50', 0) and
                    not higher_features.get('ema13_above_ema50', 0))
            else 0.0
        )
    else:
        for k in htf_keys:
            merged[f'htf_{k}'] = 0.0
        for k in ['mtf_trend_aligned','mtf_trend_conflict',
                  'mtf_confluence_bull','mtf_confluence_bear',
                  'mtf_ema_bull','mtf_ema_bear']:
            merged[k] = 0.0
